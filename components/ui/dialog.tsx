...
>(({ className, children, ...props }, ref) => (
  <DialogPortal>
    ...
    <DialogPrimitive.Content
      ref={ref}
      className={cn(
        "fixed left-[50%] top-[50%] z-50 w-[90vw] max-w-lg translate-x-[-50%] translate-y-[-50%] rounded-3xl bg-white p-8 shadow-hero outline-none",
        className
      )}
      {...props}
    >
      ...
      <DialogPrimitive.Close
        className="absolute right-4 top-4 rounded-full p-2 text-muted transition hover:bg-black/5"
        aria-label="Close"
      >
        <X className="h-4 w-4" />
      </DialogPrimitive.Close>
